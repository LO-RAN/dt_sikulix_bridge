...
# override of the original class to have response_time as an option, allowing for the creation of tests without results
...
class ThirdPartySyntheticTestsService:
    ...
    def report_simple_thirdparty_synthetic_test(
        ...
        synthetic_location = SyntheticTestLocation(
            self.__http_client, location_id)

        if detailed_steps is None:
            detailed_steps = [SyntheticTestStep(
                self.__http_client, 1, step_title)]

        monitor = ThirdPartySyntheticMonitor(
            self.__http_client, test_id, test_title, [
                synthetic_location], schedule_interval, steps=detailed_steps, edit_link=edit_link,
        )

        # only if no detailed step results were provided BUT an overall response_time was provided
        if detailed_step_results is None and response_time is not None:
            # create a default step result from this provide response_time
            detailed_step_results = [SyntheticMonitorStepResult(
        ...
        tests = ThirdPartySyntheticTests(self.__http_client, engine_name, timestamp, [
                                         location], [monitor], result_list, synthetic_engine_icon_url=icon_url)
        return tests.post()
    ...
    def report_simple_thirdparty_synthetic_test_event(
        ...
        if state == "open":
            opened_events.append(ThirdPartyEventOpenNotification(
                self.__http_client, test_id, event_id, name, event_type, reason, timestamp, [location_id]))
        else:
            resolved_events.append(ThirdPartyEventResolvedNotification(
...
class ThirdPartySyntheticTests(DynatraceObject):
    def __init__(
        ...
        raw_element = {
            ...
            "messageTimestamp": int(message_timestamp * 1000),
